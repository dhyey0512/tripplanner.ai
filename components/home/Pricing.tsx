  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2">
        <h2 className="text-yellow-500 text-center text-lg font-bold tracking-wide">Pricing</h2>
  ...
  return (
    <article
      className="flex flex-col justify-between flex-1 min-w-96 md:min-w-80
                       ring-1 ring-muted rounded-3xl p-8 shadow-xl shadow-yellow-200 dark:shadow-none
                       "
    >
      <div className="flex flex-col">
        <div>{isFreePlan ? "Free" : "Paid"} Plan</div>
        <h3 id="tier-standard" className="text-yellow-500 text-2xl font-extrabold leading-8">
        ...
        <ul role="list" className="mt-8 space-y-3 text-sm leading-6 text-foreground xl:mt-10">
          {features.map((faature, index) => (
            <li className="flex gap-x-3 text-base" key={index}>
              <CheckCircle2 className="h-6 w-5 flex-none text-yellow-600" />
              {faature}
            </li>
          ))}
        </ul>
